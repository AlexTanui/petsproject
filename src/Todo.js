  ...
  return (
    <div>
      <form onSubmit={handleNewTodoSubmit}>
        ...
        <button class="btn btn-outline-secondary"  id="button-addon2" type="submit">{editingTodoId ? "Save" : "Add Todo"}</button>
      </form>
        ...
        <tbody>
          {todos.map((todo) => (
            <tr  key={todo.id}>
              ...
              <td>
                <button id="edit_btn" type="button" class="btn btn-success" onClick={() => handleTodoEdit(todo.id)}>Edit</button>
                <button  type="button" class="btn btn-danger" onClick={() => handleTodoDelete(todo.id)}>
                  Delete
                </button>
              </td>
            </tr>
